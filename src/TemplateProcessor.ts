...
export class TemplateProcessor {
    ...
    private UseHTMLCecorators = false;
    ...
    private DirtyTemplate = false;
    private TemplateVariables = { 'Columns': [] }; // varibles in json area
    private SelectedTemplate = null;
    /* Structured content business */
    private Marking = [  { markers : [ '!===========', '===========', '===========!' ], 'type' : 'replace' },
    ...
    private Nos = new Array();
    private Level = 0;

    constructor(private context: vscode.ExtensionContext) {
        console.log('AJP - Registring template processor commands ...');
		context.subscriptions.push(
            vscode.commands.registerCommand( 'templateProcessor.enable', this.enable ),
    ...
    public processTemplate(document: TextDocument)
    {
        ...
        if (onFormat)
        {
            ...
            fs.writeFile(outputPath, this.RemoveFormat(processed), { flag: 'w' }, function (err) {
                if (err) 
                {
                    throw err;
                }
    ...
    Reduce(CurrentF, content, noFormat_offset) {
        console.log('Reduce: '+ CurrentF['name']);
        if(CurrentF['name'] == 'Style')
        {
            const s6 = '';
        }
        let replacement = '';
        ...
        if (CurrentF.positions.length > 2) {
            expression = content.substr(CurrentF.positions[ref_index],
            CurrentF.positions[CurrentF.positions.length - 2] - CurrentF.positions[ref_index]);
        }
        let offset = 0;
        for (let i = 0; i < CurrentF.children.length; i++) {
            ...
            if (CurrentF.children[i].type === 0) {
                ...
                for (let k = 0; k < varsOrder.length; k++) {
                    ...
                    replacement += this.Reduce(CurrentF.children[i], content,
                    (CurrentF.children[i].type === 2 || CurrentF.children[i].type === 3 ? CurrentF.children[i].positions[2] : noFormat_offset));
                ...
                expression = expression.substr(0,
                    offset + (CurrentF.children[i].positions[0] - CurrentF.positions[ff1]))
        ...
        switch (CurrentF.type) {
            ...
            case 1:
                ...
                replacement = this.ReplaceProperties(expression, CurrentF.variables,
                (noFormat_offset > 0));
                break;
            case 2:
                // @~ Name=='T1' ? ~@~ Yes ~@~ No ~@
            ...
            case 4:
                ...
                let transpiled = ts.transpile(this.RemoveFormat(expression));
                replacement = eval(transpiled);
                // runnalbe.Run("RUN!").then((result:string)=>{replacement = result});
                break;
        }
        return replacement;
    }
    ...
    ReplaceProperties(expression, variables, noFormat) {
        console.log('ReplaceProperties: ' + expression);        
        if(expression == 'Referencing_Table')
        {
            const s6 = '';
        }
    ...
    EvaluateConditional(expression, CurrentF) {
        let pureConditional = false;  let fragments = expression.split('~@~');
        try {
